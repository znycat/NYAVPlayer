...
/** 滑块 */
@property (nonatomic, strong) NYSliderButton *sliderBtn;
...
- (void)layoutSubviews {
    ...
    if (self.sliderBtn.hidden) {
        min_w = self.bgProgressView.ny_width * self.value;
    } else {
        min_w = self.sliderBtn.ny_centerX;
    }
    min_h = self.sliderHeight;
    ...
    min_x = 0;
    min_y = 0;
    min_w = self.bgProgressView.ny_width * self.bufferValue;
    min_h = self.sliderHeight;
    ...
    self.bgProgressView.ny_centerY     = min_view_h * 0.5;
    self.bufferProgressView.ny_centerY = min_view_h * 0.5;
    self.sliderProgressView.ny_centerY = min_view_h * 0.5;
    self.sliderBtn.ny_centerY          = min_view_h * 0.5;
}
...
- (void)setValue:(float)value {
    ...
    _value = value;
    if (self.sliderBtn.hidden) {
        self.sliderProgressView.ny_width = self.bgProgressView.ny_width * value;
    } else {
        self.sliderBtn.ny_centerX = self.bgProgressView.ny_width * value;
        self.sliderProgressView.ny_width = self.sliderBtn.ny_centerX;
    }
}
...
- (void)setBufferValue:(float)bufferValue {
    ...
    self.bufferProgressView.ny_width = self.bgProgressView.ny_width * bufferValue;
}
...
- (void)setSliderHeight:(CGFloat)sliderHeight {
    ...
    self.bgProgressView.ny_height     = sliderHeight;
    self.bufferProgressView.ny_height = sliderHeight;
    self.sliderProgressView.ny_height = sliderHeight;
}
...
- (void)setIsHideSliderBlock:(BOOL)isHideSliderBlock {
    ...
    if (isHideSliderBlock) {
        ...
        self.bgProgressView.ny_left     = 0;
        self.bufferProgressView.ny_left = 0;
        self.sliderProgressView.ny_left = 0;
        self.allowTapped = NO;
    }
}
...
- (void)startAnimating {
    ...
    scaleAnimation.toValue = @(self.ny_width * 10);
...
- (void)sliderBtnDragMoving:(UIButton *)btn point:(CGPoint)touchPoint {
    ...
    CGFloat value = (point.x - btn.ny_width * 0.5) / self.bgProgressView.ny_width;
    // value的值需在0-1之间
...
- (void)tapped:(UITapGestureRecognizer *)tap {
    ...
    // 获取进度
    CGFloat value = (point.x - self.sliderBtn.ny_width * 0.5) * 1.0 / self.bgProgressView.ny_width;
...
- (NYSliderButton *)sliderBtn {
    if (!_sliderBtn) {
        _sliderBtn = [NYSliderButton buttonWithType:UIButtonTypeCustom];
