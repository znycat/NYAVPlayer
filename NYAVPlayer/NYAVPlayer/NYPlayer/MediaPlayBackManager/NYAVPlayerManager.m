...
@interface NYPlayerLayerView : NYPlayerView
...
@implementation NYPlayerLayerView
...
@interface NYAVPlayerManager () {
    id _timeObserver;
    id _itemEndObserver;
    NYKVOController *_playerItemKVO;
}
...
@implementation NYAVPlayerManager
...
- (instancetype)init {
    self = [super init];
    if (self) {
        _scalingMode = NYPlayerScalingModeAspectFit;
        _shouldAutoPlay = YES;
    }
    return self;
}
...
- (void)prepareToPlay {
    ...
    self.loadState = NYPlayerLoadStatePrepare;
...
- (void)play {
    if (!_isPreparedToPlay) {
        ...
        self->_isPlaying = YES;
        self.playState = NYPlayerPlayStatePlaying;
    }
}
...
- (void)pause {
    [self.player pause];
    self->_isPlaying = NO;
    self.playState = NYPlayerPlayStatePaused;
...
- (void)stop {
    ...
    self.loadState = NYPlayerLoadStateUnknown;
    ...
    self.playState = NYPlayerPlayStatePlayStopped;
}
...
- (void)initializePlayer {
    ...
    NYPlayerLayerView *layerView = (NYPlayerLayerView *)self.view;
    layerView.player = _player;
...
- (void)bufferingSomeSecond {
    ...
    if (self.isBuffering || self.playState == NYPlayerPlayStatePlayStopped) return;
    /// 没有网络
...
- (void)itemObserving {
    [_playerItemKVO safelyRemoveAllObservers];
    _playerItemKVO = [[NYKVOController alloc] initWithTarget:_playerItem];
    ...
    _timeObserver = [self.player addPeriodicTimeObserverForInterval:interval queue:dispatch_get_main_queue() usingBlock:^(CMTime time) {
        ...
        if (CMTimeGetSeconds(time) > 0 && !self.isReadyToPlay) {
            ...
            self.loadState = NYPlayerLoadStatePlaythroughOK;
        }
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        if ([keyPath isEqualToString:kStatus]) {
            if (self.player.currentItem.status == AVPlayerItemStatusReadyToPlay) {
                /// 第一次初始化
                if (self.loadState == NYPlayerLoadStatePrepare) {
                ...
                NSArray *loadedRanges = self.playerItem.seekableTimeRanges;
                if (loadedRanges.count > 0) {
                    /// Fix https://github.com/renzifeng/NYPlayer/issues/475
                    if (self.playerPlayTimeChanged) self.playerPlayTimeChanged(self, self.currentTime, self.totalTime);
                }
            } else if (self.player.currentItem.status == AVPlayerItemStatusFailed) {
                self.playState = NYPlayerPlayStatePlayFailed;
            ...
            if (self.playerItem.playbackBufferEmpty) {
                self.loadState = NYPlayerLoadStateStalled;
                [self bufferingSomeSecond];
            }
        } else if ([keyPath isEqualToString:kPlaybackLikelyToKeepUp]) {
            // When the buffer is good
            if (self.playerItem.playbackLikelyToKeepUp) {
                self.loadState = NYPlayerLoadStatePlayable;
...
- (UIView *)view {
    if (!_view) {
        _view = [[NYPlayerLayerView alloc] init];
    }
    return _view;
}
...
- (void)setPlayState:(NYPlayerPlaybackState)playState {
    _playState = playState;
    if (self.playerPlayStateChanged) self.playerPlayStateChanged(self, playState);
}

- (void)setLoadState:(NYPlayerLoadState)loadState {
...
- (void)setScalingMode:(NYPlayerScalingMode)scalingMode {
    _scalingMode = scalingMode;
    NYPlayerLayerView *presentView = (NYPlayerLayerView *)self.view;
    switch (scalingMode) {
        case NYPlayerScalingModeNone:
            presentView.videoGravity = AVLayerVideoGravityResizeAspect;
            break;
        case NYPlayerScalingModeAspectFit:
            presentView.videoGravity = AVLayerVideoGravityResizeAspect;
            break;
        case NYPlayerScalingModeAspectFill:
            presentView.videoGravity = AVLayerVideoGravityResizeAspectFill;
            break;
        case NYPlayerScalingModeFill:
